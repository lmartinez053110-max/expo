...
exports.tuplesToEncodedMap = tuplesToEncodedMap;
exports.decodedMapToTuples = decodedMapToTuples;
exports.rawMappingsToTuples = rawMappingsToTuples;
...
function readSourceMapInfo(module, options) {
    ...
    if (!packed) {
        ...
        if ((0, packedMap_1.isPackedWire)(map)) {
            packed = packedMap_1.PackedMap.fromWire(map);
            map = undefined;
        }
    }
    return {
...
function tuplesToEncodedMap(opts) {
...
function decodedMapToTuples(input) {
    ...
    for (let lineIdx = 0; lineIdx < decoded.length; lineIdx++) {
        ...
        for (const seg of line) {
            ...
            if (segMut.length === 1) {
                ...
                const genCol = segMut[0];
                segMut[0] = genLine;
                segMut[1] = genCol;
                segMut[2] = segMut[2] + 1;
                if (segMut.length === 5) {
                    segMut[4] = input.names[segMut[4]];
                }
            }
            tuples.push(segMut);
        }
...
function rawMappingsToTuples(rawMappings) {
    const tuples = [];
